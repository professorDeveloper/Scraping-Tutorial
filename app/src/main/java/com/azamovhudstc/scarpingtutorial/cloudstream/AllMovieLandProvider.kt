...
class AllMovieLandProvider : MainAPI() { // all providers must be an instance of MainAPI
    ...
    suspend fun getM3u8(file: String?): String {
        println("$playerDomain/playlist/$file.txt")
        return app.post(
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        ...
        val posterUrl = fixUrlNull(
            mainUrl + this.select("div.new-short__poster > a.new-short__poster--link > img")
                .attr("data-src")
        )
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val id =
            idRegex.find(doc.select("div.tabs__content script").toString())?.groups?.get(2)?.value
