...
import com.azamovhudstc.scarpingtutorial.helper.HlsPlaylistParser.C.WIDEVINE_UUID
    ...
    private fun parseDrmSchemeData(
        ...
        if (KEYFORMAT_WIDEVINE_PSSH_BINARY == keyFormat) {
            ...
            return SchemeData(
                ...
                data = Base64.getDecoder().decode(uriString.substring(uriString.indexOf(',')))
            )
            ...
            val uriString = parseStringAttr(line, REGEX_URI, variableDefinitions)
            val data: ByteArray =
                Base64.getDecoder().decode(uriString.substring(uriString.indexOf(',')))
