...
fun main() {
    val selectedTrailer = searchMovieByVideoFon("Wednesday")[0]
...
fun getMovieVideoLink(data: SearchItem): String {
    ...
    val playlistId = doc.select("#mf-jwplayer-playlist-extra")
        .firstOrNull()?.attr("data-playlist-id")

    if (playlistId != null) {
        val videoData = VideoData(playlistId = playlistId)
        println("2-ko'rinishdan topildi: $videoData")
        m3u8Link += videoData.playlistId+".m3u8"
        return m3u8Link
    } else {
        return ""
    }
